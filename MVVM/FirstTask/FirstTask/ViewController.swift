...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let originalImage = info[.originalImage] as? UIImage {
            registrationViewModel.isBinding = Binding(originalImage)
            self.selectPhotoButton.setTitle("", for: .normal)
            self.selectPhotoButton.setBackgroundImage(originalImage, for: .normal)
        }
    ...
    fileprivate func setupRegistrationViewModelObserver() {
        registrationViewModel.isFormValidObserver = { bir in
            if (bir){
    ...
    fileprivate func setupGradientLayer() {
        ...
        let bottomColor = #colorLiteral(red: 0.1215686277, green: 0.01176470611, blue: 0.4235294163, alpha: 1)
